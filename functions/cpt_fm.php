...
function property_general_info_html($post) {
    ...
    case 'float':
        ...
        echo "<input type='number' name='$field' value='" . esc_attr($value) . "' step='0.01' />";
        break;
    ...
    if (isset($_POST[$field])) {
        ...
        if ($field === 'rera_registered' || $field === 'power_backup' || $field === 'pet_friendly' || $field === 'internet') {
            ...
            switch ($field) {
                ...
                case 'discount':
                case 'pincode':
                    // These are number fields, sanitize them as integers (pincode is also numeric)
                    update_post_meta($post_id, $field, intval($_POST[$field]));
                    break;

                case 'latitude':
                case 'longitude':
                    // These are float fields, sanitize them as floats
                ...
                default:
                    // If the field is not recognized, treat it as text and sanitize
                    update_post_meta($post_id, $field, sanitize_text_field($_POST[$field]));
                    break;
            }
        }
