...
function render_property_search_section($group_by = 'location') {
    ...
    // Ensure group_by has a valid value (location, status, or type)
    if (!in_array($group_by, ['location', 'status', 'type'])) {
        $group_by = 'location'; // Default to 'location' if invalid
    }
    ...
    // Remove tax_query if it's empty (optimization)
    ...
    if ($property_query->have_posts()) :
        ...
        // Group properties by the selected criterion (location, status, type)
        $grouped_properties = [];
        foreach ($properties as $property) {
            $group_key = '';
            if ($group_by === 'location') {
                $group_key = $property['location'];
            } if ($group_by === 'status') {
                $group_key = $property['status'];
            } if ($group_by === 'type') {
                // Assuming 'type' is a custom taxonomy
                $property_types = get_the_terms($property['id'], 'property_type');
                $group_key = $property_types ? wp_list_pluck($property_types, 'name')[0] : 'Unknown Type';
            }

            $grouped_properties[$group_key][] = $property;
        }

        // Render the grouped properties
        ?>
        <!-- Start of the Section -->
        <section class="py-3">
            <h2 class="container">Grouped By <?php echo ucfirst($group_by); ?></h2>
    ...
    ?>
        ...
        <div class="card property-card shadow-sm">
            ...
            <div class="card-body d-flex flex-column justify-content-between">
                ...
                <div class="btn-group">
                    <a href="<?php the_permalink(); ?>" class="btn btn-outline-primary btn-sm mt-3">View Details</a>
                    <a href="<?php the_permalink(); ?>" class="btn btn-outline-primary btn-sm mt-3">Contact</a>
                </div>
            </div>
        </div>
